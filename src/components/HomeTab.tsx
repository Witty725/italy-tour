...
import { Plane, Compass, Sparkles, CheckSquare, HelpCircle, MessageSquareCode, CalendarRange, Heart } from 'lucide-react';
...
export function HomeTab({ onNavigate }: HomeTabProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Bento Grid Stats & Interactive Fun Facts */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Packing Quick-look Widget */}
        <motion.div
          initial={{ opacity: 0, scale: 0.98 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.4, delay: 0.15 }}
          className="glass-panel p-5 flex flex-col justify-between border-emerald-500/20"
        >
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-emerald-400 text-xs font-black bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                {packingPercent}% Ready
              </span>
            </div>

            <div className="flex items-center gap-4 py-1">
              <div className="relative w-16 h-16 sm:w-18 sm:h-18 rounded-full border-4 border-slate-850 flex items-center justify-center shrink-0">
                {/* SVG Circle Progress indicator */}
                <svg className="absolute -rotate-90 w-16 h-16 sm:w-18 sm:h-18">
                  <circle
                    cx="32"
                    cy="32"
                    r="26"
                  ...
                  <circle
                    ...
                    strokeDashoffset={2 * Math.PI * 26 * (1 - packingPercent / 100)}
                  />
                </svg>
                <span className="text-xs font-mono text-slate-200 font-extrabold">{packedCount}/{packingList.length}</span>
              </div>
              
              <div className="flex flex-col">
                <span className="text-slate-250 text-xs font-extrabold leading-tight">
                  {packedCount === packingList.length ? 'Hooray, fully loaded!' : 'Keep packing those essentials!'}
                </span>
                <span className="text-[11px] text-slate-500 leading-snug mt-1">
                  Double check critical documents, medications, and appropriate Vatican clothes before departing.
                </span>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="mt-4 w-full py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-lg border border-emerald-500/20 transition-all flex items-center justify-center gap-1.5"
          >
            Open Backing Checklist
          </button>
        </motion.div>
      </div>
